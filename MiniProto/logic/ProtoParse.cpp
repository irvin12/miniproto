...
(
str_p("bool") |
str_p("int32") | str_p("int64") | str_p("sint32") | str_p("sint64") | str_p("uint32") | str_p("uint64") |
str_p("float") | str_p("double") |
str_p("string") | str_p("bytes") |
str_p("required") | str_p("optional") | str_p("repeated") | str_p("set") | str_p("map") |
...
{
	std::string packageStr(start, end - start);
	...
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "\"" << packageStr << "\" is not valid package name. It will be skipped." << std::endl;
	}
}
void ProtoParse::ParseProtoImport(const char* start, const char* end)
{
	std::string importStr(start, end - start);
	...
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "\"" << importStr << "\" is not valid import name. It will be skipped." << std::endl;
	}
}
void ProtoParse::ParseProtoMessage(const char* start, const char* end)
{
	std::string messageStr(start, end - start);
	...
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << messageStr << " is not valid message. It will be skipped." << std::endl;
	}
}
void ProtoParse::ParseProtoMessageMember(const char* start, const char* end)
{
	std::string memberStr(start, end - start);

	// messageMember构成：
	// typeNameRule ::= {ValidProtoName , '.'} , ValidProtoName
	// typeRule1 ::= "bool" | "int32" | "sint32" | "uint32" | "int64" | "sint64" | "uint64" | "float" | "double" | "string" | "bytes" | typeNameRule
	// typeRule2 ::=          "int32" | "sint32" | "uint32" | "int64" | "sint64" | "uint64"                      | "string"
	// typeRule ::= {"map", {' '} , '<', {' '} , typeRule2 , {' '} , ',' , {' '}, typeRule1, {' '} , '>'} | 
	//				{"set", {' '} , '<', {' '} , typeRule2 ,                                 {' '} , '>'} | 
	//				{"repeated" , ' ' , typeRule1} | 
	//				{"required" | "optional" , ' ' , typeRule1} | 
	//				{typeRule1}
	...
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "\"" << m_TempStruct.m_Name << "\" : \"" << memberStr << "\" is not valid message member. It will be skipped." << std::endl;
	}
}
void ProtoParse::ParseProtoEnum(const char* start, const char* end)
{
	std::string enumStr(start, end - start);
	...
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << enumStr << " is not valid enum.It will be skipped." << std::endl;
	}
}
void ProtoParse::ParseProtoUnknown(const char* start, const char* end)
{
	std::string unknownStr(start, end - start);

	std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
	std::cout << "\"" << unknownStr << "\" is not valid code. It will be skipped." << std::endl;
}

void ProtoParse::PushPackageName(const char* start, const char* end)
{
	std::string packageName(start, end - start);
...
{
	std::string importName(start, end - start);

	int lastPos = importName.find_last_of(".");
	if (-1 != lastPos)
	{
...
{
	std::string messageName(start, end - start);
...
void ProtoParse::PushMessageMapMemberType(const char* start, const char* end)
{
	...
	std::string type(start, end - start);
...
void ProtoParse::ProcessKeyType(const std::string& type)
{
...
void ProtoParse::ProcessValueType(const std::string& type)
{
	...
	else
	{
		...
		std::string packageName = ProtoStructFullName2PackageName(type);
		if (packageName.length() == 0)
		{
			packageName = m_TempFile.m_Package;
		}
		std::string typeName = ProtoStructFullName2TypeName(type);
		...
		if (!hasType)
		{
			...
			{
				std::map<std::string, ProtoStruct>::const_iterator itStruct = m_TempFile.m_Structs.find(typeName);
		...
		if (!hasType)
		{
			if (0 != typeName.length() && 0 != packageName.length())
			{
				...
				{
					std::map<std::string, ProtoFile>::const_iterator itFile = m_Files.find(m_TempFile.m_Includes[i]);
					if (itFile != m_Files.end())
					{
						const ProtoFile& protoFile = itFile->second;
						if (0 == strcmp(packageName.c_str(), protoFile.m_Package.c_str()))
						{
							std::map<std::string, ProtoStruct>::const_iterator itStruct = protoFile.m_Structs.find(typeName);
...
void ProtoParse::PushMessageMemberEnd()
{
	...
	{
		std::map<unsigned int, MessageMember>::iterator it = m_TempStruct.m_MessageMembers.find(m_TempMessageMember.m_Tag);
		...
		{
			std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
			std::cout << "Field number " << m_TempMessageMember.m_Tag << " has already been used in \"" << m_TempStruct.m_Name << "\" by field \"" << it->second.m_Name << "\"." << std::endl;
		}
	}
	else
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "Unknown type \"" << m_TempMessageMember.m_TypeName << "\"." << std::endl;
	}
}
void ProtoParse::PushMessageEnd()
{
	...
	if (!hasType)
	{
		std::map<std::string, ProtoStruct>::const_iterator itStruct = m_TempFile.m_Structs.find(m_TempStruct.m_Name);
		if (itStruct != m_TempFile.m_Structs.end())
		{
			std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
			std::cout << "\"" << m_TempStruct.m_Name << "\" is already defined in file : " << m_TempFile.m_FileName << std::endl;
	...
	if (!hasType)
	{
		std::map<std::string, ProtoFile>::const_iterator itFile = m_Files.begin();
		while (itFile != m_Files.end())
		{
			const ProtoFile& protoFile = itFile->second;
			if (0 == strcmp(m_TempStruct.m_PackageName.c_str(), protoFile.m_Package.c_str()))
			{
				std::map<std::string, ProtoStruct>::const_iterator itStruct = protoFile.m_Structs.find(m_TempStruct.m_Name);
				if (itStruct != protoFile.m_Structs.end())
				{
					std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
					std::cout << "\"" << m_TempStruct.m_Name << "\" is already defined in file : " << protoFile.m_FileName << std::endl;
...
{
	std::string valueStr(start, end - start);
	...
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "\"" << m_TempStruct.m_Name << "\" : \"" << valueStr << "\" is not valid enum value.It will be set as last entry + 1." << std::endl;
...
{
	std::map<unsigned int, std::string>::reverse_iterator itOrder = m_TempStruct.m_EnumMemberOrder.rbegin();
	if (itOrder != m_TempStruct.m_EnumMemberOrder.rend())
	{
		std::map<std::string, EnumMember>::iterator itEntry = m_TempStruct.m_EnumMembers.find(itOrder->second);
...
{
	std::map<std::string, EnumMember>::iterator it = m_TempStruct.m_EnumMembers.find(m_TempEnumMember.m_Name);
	...
	{
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "\"" << m_TempEnumMember.m_Name << "\"" << "is already defined in \"" << m_TempStruct.m_Name << "\"." << std::endl;
	}
}
...
void ProtoParse::PushEnum(const char* start, const char* end)
{
	...
	// 搜索已存在的proto结构信息，搜不到则视为未定义类型
	std::string packageName = ProtoEnumFullName2PackageName(enumName);
	if (packageName.length() == 0)
	{
		packageName = m_TempFile.m_Package;
	}
	std::string typeName = ProtoEnumFullName2TypeName(enumName);
	if (typeName.length() == 0)
	{
		typeName = m_TempStruct.m_Name;
	}
	std::string entryName = ProtoEnumFullName2EntryName(enumName);
	...
	if (!hasType)
	{
		...
		{
			std::map<std::string, EnumMember>::const_iterator itEntry = m_TempStruct.m_EnumMembers.find(entryName);
			if (itEntry != m_TempStruct.m_EnumMembers.end())
			{
	...
	if (!hasType)
	{
		if (0 != entryName.length() && 0 != typeName.length() && (0 == packageName.length() || 0 == strcmp(packageName.c_str(), m_TempFile.m_Package.c_str())))
		{
			...
			if (itStruct != m_TempFile.m_Structs.end())
			{
				...
				std::map<std::string, EnumMember>::const_iterator itEntry = protoStruce.m_EnumMembers.find(entryName);
	...
	if (!hasType)
	{
		...
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "\"" << enumName << "\" is not defined." << std::endl;
	}
}
void ProtoParse::Add()
{
...
void ProtoParse::Div()
{
	...
	else
	{
		...
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : " << std::endl;
		std::cout << "\"" << m_TempEnumMember.m_Name << "\" has a arithmetic exception!" << std::endl;
	}
}
void ProtoParse::Mod()
{
...
std::string ProtoParse::ProtoStructFullName2PackageName(const std::string& fullName)
{
...
std::string ProtoParse::ProtoStructFullName2TypeName(const std::string& fullName)
{
	int index = fullName.find_last_of('.');
	if (-1 != index)
	{
...
std::string ProtoParse::ProtoEnumFullName2PackageName(const std::string& fullName)
{
	int index = fullName.find_last_of('.');
	if (-1 != index)
	{
		std::string fullStr1 = fullName.substr(0, index);
...
std::string ProtoParse::ProtoEnumFullName2TypeName(const std::string& fullName)
{
	int index = fullName.find_last_of('.');
	if (-1 != index)
	{
...
std::string ProtoParse::ProtoEnumFullName2EntryName(const std::string& fullName)
{
	int index = fullName.find_last_of('.');
	if (-1 != index)
	{
...
void ProtoParse::ParseFile(const char* fullPathName)
{
	...
	if (m_TempFile.m_FileName.length() > 0)
	{
		...
		int lastPos = m_TempFile.m_FileName.find_last_of("/");
		if (0 <= lastPos)
		{
	...
	txt = StringTool::ReplaceReg(txt, "[/][*](.*?)[*][/]", "");	// 去除段注释/**/
	...
	if (txt.length() > 0)
	{
		...
		std::cout << "File : \"" << m_TempFile.m_FileName << "\" : finish." << std::endl;
	}
	else
	{
		return;
