...
void SetMsgTestArray(common::proto2::MsgTestArray<>& msg, size_t n)
{
	msg.Clear();
...
void SetMsgTestSet(common::proto2::MsgTestSet<>& msg, size_t n)
{
	msg.Clear();
	for (size_t i = 0; i < n; i++)
	{
...
void SetMsgTestMap(common::proto2::MsgTestMap<>& msg, size_t n)
{
...
void SetMsgTestMap(const common::proto2::MsgTestMap<>& msg, size_t n)
{
	for (size_t i = 0; i < n; i++)
	{
		char tempK[10] = { 0 };
...
void SetMsgTest(common::proto2::MsgTest<>& msg, size_t n)
{
	{
	...
	}
	{
		...
		for (size_t i = 0; i < msg.m_testsingular_array_size(); i++)
		{
	...
	}
	{
		...
		for (size_t i = 0; i < msg.m_testarray_array_size(); i++)
		{
	...
	}
	{
		...
		for (size_t i = 0; i < msg.m_testset_array_size(); i++)
		{
	...
	}
	{
		...
		for (size_t i = 0; i < msg.m_testmap_array_size(); i++)
		{
...
int main(int argc, char* argv[])
{
	...
	if (true)
	{
		...
		common::miniproto::byte_size input1BuffSize = inputMsg1.ByteSize();
		common::miniproto::byte_size input2BuffSize = inputMsg2.ByteSize();

		// 序列化到buff
		if (!inputMsg1.SerializeToArray((unsigned char *)input1Buff, input1BuffSize))
		{
			std::cout << "input1Msg SerializeToArray error." << std::endl;
		}
		if (!inputMsg2.SerializeToArray((unsigned char *)input2Buff, input2BuffSize))
		{
		...
		if (output1Stream)
		{
			output1Stream.write(input1Buff, input1BuffSize);
			output1Stream.close();
		}
		output2Stream.open("./output2Buff.txt", std::ios::out | std::ios::binary | std::ios::trunc);
		if (output2Stream)
		{
			output2Stream.write(input2Buff, input2BuffSize);
			output2Stream.close();
		}

		// 从buff反序列化
		if (!outputMsg.ParseFromArray((unsigned char *)input1Buff, input1BuffSize))
		{
			std::cout << "outputMsg ParseFromArray error." << std::endl;
		}
		if (!outputMsg.ParseFromArray((unsigned char *)input2Buff, input2BuffSize))
		{
			std::cout << "outputMsg ParseFromArray error." << std::endl;
		}
	}
